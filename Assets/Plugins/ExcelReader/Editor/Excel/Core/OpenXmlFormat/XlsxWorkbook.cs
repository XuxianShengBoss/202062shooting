...
namespace Excel.Core.OpenXmlFormat
{
    internal class XlsxWorkbook
    {
		...
		private const string N_rPh = "rPh";//LYH add for remove pinyin
        ...
        private void ReadSharedStrings(Stream xmlFileStream)
        {
			...
			//LYH Add to remove pinyin
			int l = (int)xmlFileStream.Length;
			byte[] d = new byte[l];
			xmlFileStream.Read (d,0,l);
			xmlFileStream.Close ();
			// to string
			string str = System.Text.Encoding.UTF8.GetString (d);

			//UnityEngine.Debug.LogError (str);

			str=System.Text.RegularExpressions.Regex.Replace(str, "<rPh [\\s\\S]*?</rPh>", "");


			//UnityEngine.Debug.LogError (str);


			xmlFileStream = new MemoryStream (System.Text.Encoding.UTF8.GetBytes(str));
			//LYH add end
